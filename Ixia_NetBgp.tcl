...
class BgpSession {
    ...
    constructor { port  { hBgpSession NULL }  } {
        ...
        Deputs "portObj returned $portObj port object is  $port"
		set handle ""
        #set ip_version $version
		set routeBlock(obj) [list]
        
        #set portObj [ GetObject $port ]
        set handleObj [$portObj cget -handle]
        Deputs "value of cget handle is $handleObj"
        if { [ catch {
        ...
        if { $hBgpSession != "NULL" } {
            ...
            if { $handle != "" } {
                Deputs "handle received not null from GetValidHandleObj"
        ...
        if { $handle == "" } {
            set handleName $this           
            Deputs "Calling reborn in classic BGP handle received is null"
            reborn
        }
...
class SimRoute {
    ...
    constructor { bgpobj { hRouteBlock NULL } } {
        ...
        if { $hRouteBlock != "NULL" } {
            set handle [GetValidHandleObj "simroute" $hRouteBlock $hBgp]
            Deputs "----- handle: $handle -----"
            if { $handle != "" } {
                set handleName [ ixNet getA $handle -name ] 
            } else {
                error "$errNumber(5) handle:$hRouteBlock"
            }                       
        }                        		
	}
...
body BgpSession::advertise_route { args } {
	...
	if { [ info exists route_block ] } {
        ...
        if { $LoadConfigMode } {
            set routeRangeList  [ixNet getL $handle routeRange]
            if { $routeRangeList != "" } {
                foreach rRange $routeRangeList {
                    ixNet setA $rRange -enabled True
                }
                ixNet commit
            }
            set l3siteList  [ixNet getL $handle l3Site]
            if { $l3siteList != "" } {
                foreach l3Range $l3siteList {
                    set vpnRRangelist [ixNet getL $l3Range vpnRouteRange ]
                    if { $vpnRRangelist != "" } {
                        foreach vpnRRange $vpnRRangelist {
...
Deputs "Args:$args "
	...
	if { [ info exists route_block ] } {
		ixNet setA $routeBlock($route_block,handle) -enabled False
	} else {
         if { $LoadConfigMode } {
            set routeRangeList  [ixNet getL $handle routeRange]
            if { $routeRangeList != "" } {
                foreach rRange $routeRangeList {
                    ixNet setA $rRange -enabled False
                }
                ixNet commit
            }
            set l3siteList  [ixNet getL $handle l3Site]
...
body SimRoute::config { args } {
	...
	if { [ info exists route_block ] } {
		foreach rb $route_block {
			...
			Deputs "num:$num, step:$step, prefix_len:$prefix_len, start:$start, type:$type"
            if { $handle == "" } {
				set hRouteBlock [ ixNet add $hBgp routeRange ]
				ixNet commit 
				set handle $hRouteBlock
			}
			set handle [ ixNet remapIds $handle ]
            Deputs "handle:$handle"
			...
			$rb configure -hPort $hPort
			$rb configure -protocol "bgp"
            Deputs "hPort :: $hPort portObj :: $portObj handle :: $handle"
			$rb enable
		}
	}
	
	if { ! [info exists as_path_option] } {
	    set bgpType [ ixNet getA $hBgp -type ]
		if { $bgpType == "internal" } {
		    set as_path_option "noInclude"
		} else {
		    set as_path_option "includeAsSeq"
		}
	}
	...
	if { $handle != "" } {
		...
		if { [info exists enable_as_path ] } {
			ixNet setM $handle \
				-enableAsPath $enable_as_path \
				-asPathSetMode $as_path_option
			ixNet commit
		}
...
Deputs "----- TAG: $tag -----"
        ...
        Deputs "value of bgp is $bgp and value of hBgp is $hBgp"
