...
class LdpSession {
	...
	method reborn {{version ipv4}} {
        ...
        if {[llength $topoObjList] != [llength $vportList]} {
            foreach topoObj $topoObjList {
                ...
                if {$vportObj != $vport && $vport == $hPort} {					
					set ldpObj [CreateProtoHandleFromRoot $hPort "ldpBasicRouter" $ip_version]			
                }
		...
		if { [ llength $topoObjList ] == 0 } {
            if { $ip_version == "ipv4" } {
				set ldpObj [CreateProtoHandleFromRoot $hPort "ldpBasicRouter" $ip_version]
                set ipv4Obj [GetDependentNgpfProtocolHandle $ldpObj "ip"]
                if {[llength $ldpObj] == 0} {
            ...
            if { $ip_version == "ipv6" } {
				set ldpObj [CreateProtoHandleFromRoot $hPort "ldpBasicRouterV6" $ip_version]                
				set ipv6Obj [GetDependentNgpfProtocolHandle $ldpObj "ip"]
				if {[llength $ldpObj] == 0} {
		...
		#Enable protocol 		
        ixNet setA [ixNet getA $ldpObj -active]/singleValue -value True
        ixNet commit

		# -- add ldp routers 		
	    if { $ip_version == "ipv4" } {
		    set ipv4Obj [GetDependentNgpfProtocolHandle $ldpObj ip]
            #set rb_interface [GetDependentNgpfProtocolHandle $ipv4Obj "ldpConnectedInterface"]
            set rb_interface [ixNet getL $ipv4Obj "ldpConnectedInterface"]
	        Deputs "rb_interface is: $rb_interface"		
            $this configure -ipv4Handle $ipv4Obj
        } else {
		    set ipv6Obj [GetDependentNgpfProtocolHandle $ldpObj ip]
            #set rb_interface [GetDependentNgpfProtocolHandle $ipv6Obj "ldpv6ConnectedInterface"]
            set rb_interface [ixNet getL $ipv4Obj "ldpv6ConnectedInterface"]
	        Deputs "rb_interface is: $rb_interface"
	    ...
	    $this configure -ldpInterface $rb_interface
	...
	foreach int $rb_interface {
		...
		if { $version == "ipv4" } {
		    set hInt [ ixNet getL $ipv4Handle ldpConnectedInterface ]
		    ixNet setA [ixNet getA $hInt -active]/singleValue -value True		
		} else {        
		    set hInt [ ixNet getL $ipv6Handle ldpv6ConnectedInterface ]
		    ixNet setA [ixNet getA $hInt -active]/singleValue -value True
		}		
...
body LdpSession::get_stats {} {
    ...
    foreach viewObj $viewList {
        if {"::ixNet::OBJ-/statistics/view:\"LDP Per Port\"" == $viewObj} {
            set view $viewObj
        }
        if {"::ixNet::OBJ-/statistics/view:\"LDP+ Per Port\"" == $viewObj} {
            set view $viewObj
            ...
            foreach deviceGroupObj $deviceGroupObjList {
                set ethernetObj [ixNet getL $deviceGroupObj ethernet]
	                ...
	                if { [ info exists label_advertise_mode ] } {
					    ...
					    ixNet setA [ixNet getA $rb_interface -operationMode]/singleValue -value $label_advertise_mode            
						
	
	                }
					if { [ info exists hello_interval ] } {
						ixNet setA [ixNet getA $rb_interface -basicHelloInterval]/singleValue -value $hello_interval          
...
body LdpLsp::establish_lsp {} {
    ...
    Deputs "----- TAG: $tag -----"
	set ldpInfeace [GetDependentNgpfProtocolHandle $ldpObj "ldpConnectedInterface"]
	set ldpRouterList [GetDependentNgpfProtocolHandle $ldpObj "ldpBasicRouter"]
	foreach ldpRouter $ldpRouterList {
	    ixNet setA [ixNet getA $ldpRouter -active]/singleValue -value True

	}
	ixNet setA [ixNet getA $ldpInfeace -active]/singleValue -value True
	ixNet commit
}
...
body LdpLsp::teardown_lsp {} {
	...
	set ldpRouterList [GetDependentNgpfProtocolHandle $ldpObj "ldpBasicRouter"]
    foreach ldpRouter $ldpRouterList {
	    ixNet setA [ixNet getA $ldpRouter -active]/singleValue -value False

	}
	ixNet setA [ixNet getA $ldpInfeace -active]/singleValue -value False
	ixNet commit
}
...
body Ipv4PrefixLsp::config { args } {
    ...
    puts "num : $num start : $start . $step step , prefix_len : $prefix_len  "
	if { [ info exists num ] } {
...
body VcLsp::config { args } {
	...
	if { [ info exists encap ] } {
		...
		set networkGroupList [GetDependentNgpfProtocolHandle $hLdp "networkGroup"]		
        foreach networkGroupObj $networkGroupList {
	        set macPoolsObj [ixNet getL $networkGroupObj "macPools"]
