    ...
    while { 1 } {
        ...
        if { [llength $streams ] > 0 } {
            foreach stream $streams {
                set highLevelStreamList [ ixNet getL $stream configElement ]
				foreach highLevelStream $highLevelStreamList {
            ...
            foreach trafficItem [ixNet getL $traffic trafficItem] {
                set highLevelStreamList [ ixNet getL $trafficItem configElement ]
				foreach highLevelStream $highLevelStreamList {
					set frameSize [ ixNet getL $highLevelStream frameSize ]
					ixNet setA $frameSize -fixedSize $frame_size
					ixNet commit 
					set frameRate [ ixNet getL $highLevelStream frameRate ]
					set endpointSetList [ ixNet getL $trafficItem endpointSet ]
					foreach endpointSet $endpointSetList {
						...
						foreach upstream $upstreams {
							if { $upstream == [ string range $src 0 [ expr [ string length $upstream ] - 1 ] ] ||
								...
								ixNet setM $frameRate -rate $percentage -type percentLineRate
        ...
        if { [ Tester::isLossFrames $all_streams $acceptloss] } {
            set iterationResult false
        } else {
