...
# Deputs "Port connect state:[ ixNet getA $hPort -isAvailable ]"
	...
	method stop {} {
        ...
        if { $capfile != "" } {
            after 8000
            set dir    [file dirname $capfile]
            set file   [file tail $capfile]
            catch {
                ixNet exec saveCapture $dir
                after 3000
            }
            set portName [ixNet getA  [$portObj cget -handle] -name]
            set len [string length $portName]
            for { set index 0} {$index < $len } { incr idnex } {
                if { [ string index $portName $index ] == ":"  || \
                     [ string index $portName $index ] == "/" } {
    ...
    method save { args } {}
    method save_file {} {
        set tag "body Capture::save_file [info script]"
Deputs "----- TAG: $tag -----"
        if { $capfile != "" } {
            set dir    [file dirname $capfile]
            set file   [file tail $capfile]
            ...
            for { set index 0} {$index < $len } { incr index } {
                ...
                set fileCap [lindex [ glob -directory $dir -tails *{${portName}_HW}*.cap ] 0]
                file delete -force "$dir/$file"
                file rename -force "$dir/$fileCap" "$dir/$file"
            }
        }
...
Deputs "hPort:$hPort"	
        ixNet setM $hPort/capture -afterTriggerFilter captureAfterTriggerAll\
		-beforeTriggerFilter captureBeforeTriggerAll -captureMode captureContinuousMode\
		-continuousFilters captureContinuousFilter -displayFiltersDataCapture {} \
		-hardwareEnabled True -sliceSize $slice_size \
		-softwareEnabled False -triggerPosition 1
		ixNet commit
	if {[ info exists capSrc ]} {
		if {$capSrc == "rx"} {
			set hardwareEnb "True"
			set softwareEnb "False"
		} elseif {$capSrc == "tx"} {
			set hardwareEnb "False"
			set softwareEnb "True"
		} elseif {$capSrc == "tx_rx"} {
			set hardwareEnb "True"
			set softwareEnb "True"
		}	
		
		ixNet setA $hPort/capture -hardwareEnabled $hardwareEnb -softwareEnabled $softwareEnb
	}

	if { [ info exists cap_filter ] } {
Deputs "hPort:$hPort"

		ixNet setM $hPort/capture/filter -captureFilterEnable True -captureFilterError errAnyFrame 	-captureFilterFrameSizeEnable False -captureFilterFrameSizeFrom 64 	-captureFilterFrameSizeTo 1518 \
		ixNet commit
		set index 1
...
# IxDebugOn
puts "field_value:$field_value ip_offset:$ip_offset vlan_offset:$vlan_offset mpls_offset:$mpls_offset arp_offset:$arp_offset udp_offset:$udp_offset sip_offset:$sip_offset field_offset:$field_offset field_mask:$field_mask eth_dst:$eth_dst eth_src:$eth_src"
Deputs Step100
