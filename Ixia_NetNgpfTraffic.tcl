...
body Traffic::constructor { port { hTraffic NULL } } {
    ...
    Deputs "traffic :$hTraffic"
	Deputs "LoadConfigMode: $LoadConfigMode"
	Deputs "handle : $handle"
	if { $hTraffic != "NULL" } {
		Deputs "traffic is Null "
		set handle $hTraffic
...
body Traffic::config { args  } {
    ...
    foreach { key value } $args {
        ...
        switch -exact -- $key {
            ...
            -vlan_priority_tracking {
                set value [BoolTrans $value]
                set vlan_priority_tracking $value
            }
			-mpls_exp_tracking {
                set value [BoolTrans $value]
                set mpls_exp_tracking $value
			}
            -no_mesh {
                set value [BoolTrans $value]
                set no_mesh $value
			...
			-enable_mixed_speed {
				set enable_mixed_speed true
			}
			-flow_group_based_on {
				set flow_group_based_on $value
			}
            default {
    ...
    if { [ info exists src ] && [ info exists dst ] } {
        if { [ IsIPv4Address $src ] && [ IsIPv4Address $dst ] } {
            #-- quick stream IPv4
            Deputs "waseemTraffic type:quick stream IPv4"
            #-- Create quick stream
            ...
            foreach srcEndpoint $src {
                ...
                if { ( [ $srcObj isa Port ] == 0 ) && ( [ $srcObj isa EmulationNgpfObject ] == 0 ) && ( [ $srcObj isa Host ] == 0 ) } {
                ...
                if { [ $srcObj isa Port ] } {
                    ...
                    set trafficType [ $srcObj cget -type ]
					Deputs "WaseemtrafficType $trafficType"
            ...
            foreach dstEndpoint $dst {
                ...
                if { ( [ $dstObj isa Port ] == 0 ) && ( [ $dstObj isa EmulationNgpfObject ] == 0 ) && ( [ $dstObj isa Host ] == 0 ) } {
                ...
                if { [ $dstObj isa Port ] } {
                    ...
                    set dstHandle [ concat $dstHandle [ $dstObj cget -handle ] ]
                    lappend dstPortHandle [ $dstObj cget -hPort ]
					Deputs "waseemdst $dstHandle $dstPortHandle"
             ...
             Deputs "dstPort handle:$dstPortHandle"
			...
			if {[info exists enable_mixed_speed] && [regexp -nocase "true" $enable_mixed_speed]} {
				...
				for {set i 0} {$i < [llength $srcPortHandle]} {incr i} {
					...
					if { $currentType == "ethernetvm" || $currentType == "ethernet" } {
						...
						if {[regexp -nocase novusHundredGigLan $currentType]} {
							set currentType hundredGigLan
						}
				...
				foreach speed [array names srcPortSpeedArr] {
					if {[catch {
						Deputs "name: $speed"
						#Deputs "srcPortSpeedArr $srcPortSpeedArr"
						Deputs "src:$srcHandle"
						Deputs "dst:$dstHandle"
						Deputs "handle:$handle"
			...
			Deputs "endpointSetList:$endpointSetList"
        }
		...
		foreach hStream $highLevelStream {
            ...
            if { $flagPduObj } {
                ...
                foreach name $pdu {
                    ...
                    Deputs "type: $type "
                    if { $pindex == 0 } {
                        if { ([info exists src] == 0 ) || ( [info exists dst] == 0 ) } {
                            $name ChangeType SET
                            set type [ string toupper [ $name cget -type ] ]
                        }
                    }
                    ...
                    if { $type == "APPLIST" } {
                        ...
                        foreach single $objList stack $listStack {
                            ...
                            set enables [ $single cget -enables ]
	...
	if { $to_raw } {
		...
		if {$LoadConfigMode} {
			set name [ixNet getA $oldHandle -name]
			ixNet setA $handle -name $name
		} else {
			ixNet setA $handle -name $this
		}
		ixNet commit
                        ...
                        APPLIST {
                            ...
                            foreach single $objList {					                                 
                                if { $listStack == ""} {
                                    ...
                                    lappend listStack [lindex $stacklist $index]
                                    incr index                                    
                                }
    ...
    if { [ info exists boun_pdu ] } {
        
		foreach hStream $highLevelStream {
            set flagPduObj  1
            Deputs "boun_pdu:$boun_pdu"
            foreach head $boun_pdu {
                Deputs "head:$head"
                set head [ GetObject $head ]
                Deputs "head obj:$head"
                if { $head != "" } {
            ...
            if { $flagPduObj } {
                ...
                foreach name $boun_pdu {
                    ...
                    if { $findpdu == 0 } {
                        ...
                        set stackList [ ixNet getList $hStream stack ]
                        Deputs "Stack list:$stackList"
                        set appendHeader [ lindex $stackList [expr [llength $stackList] - 2] ]
                        Deputs "appendHeader:$appendHeader"
                        Deputs "stack to be added: $proStack"
                        ixNet exec append $appendHeader $proStack
                        set stackList [ ixNet getList $hStream stack ]
                        set pro [lindex $stackList [expr [llength $stackList] - 2]]
                        Deputs "pro:$pro"
    ...
    if { [ info exists tx_mode ] } {
        ...
        foreach configElement $highLevelStream {
			Deputs "configuring Txnode $configElement"
    ...
    if { [ info exists tx_num ] } {
		foreach configElement $highLevelStream {
			...
			Deputs "configuring Tx num $configElement"
    ...
    if { [ info exists frame_len_type ] } {
		...
		foreach configElement $highLevelStream {
			Deputs "waseem test3"
			Deputs "configuring frame_len_type $configElement"
    ...
    if { [ info exists frame_len ] } {
		...
		foreach configElement $highLevelStream {
            if { $frame_len_type == "weightedPairs"} {          
                ...
                ixNet setA $configElement/frameSize -weightedPairs $frameLenList -weightedRangePairs $frameLenList
				ixNet commit
            } else {
    ...
    if { [ info exists precedence_tracking ] 
		...
		if {![info exists flow_group_based_on] || [info exists flow_group_based_on] && [regexp -nocase "precedence" $flow_group_based_on]} {
			foreach configElement $highLevelStream {
    ...
    if { [ info exists vlan_priority_tracking ] 
		&& $vlan_priority_tracking } {
        Deputs "set vlan priority tracking..."
		set trackBy [ ixNet getA $handle/tracking -trackBy ]
		lappend trackBy vlanVlanUserPriority0
		ixNet setA $handle/tracking -trackBy $trackBy
		if {![info exists flow_group_based_on] || [info exists flow_group_based_on] && [regexp -nocase "vlanpriority" $flow_group_based_on]} {
			foreach configElement $highLevelStream {
				ixNet setA $configElement/transmissionDistribution \
					-distributions vlanVlanUserPriority0
					ixNet commit
			}
		}
	}
	if { [ info exists mpls_exp_tracking ] 
		&& $mpls_exp_tracking } {
        Deputs "set mpls exp tracking..."
		set trackBy [ ixNet getA $handle/tracking -trackBy ]
		lappend trackBy mplsMplsExp0
		ixNet setA $handle/tracking -trackBy $trackBy
		if {![info exists flow_group_based_on] || [info exists flow_group_based_on] && [regexp -nocase "mplsexp" $flow_group_based_on]} {
			foreach configElement $highLevelStream {
				ixNet setA $configElement/transmissionDistribution \
					-distributions mplsMplsExp0
					ixNet commit
			}
		}
	}
    ...
    foreach hs $highLevelStream {
        foreach stack [ ixNet getList $hs stack ] {
            ...
            } elseif { [ regexp vlan $stack ] } {
				Deputs "waseem etst9"
				foreach filed [ ixNet getList $stack field ] {
					 if { [ regexp -nocase "vlanUserPriority" $filed ] } {
						if { [ info exists vlan_priority ] } {
							if { [llength $vlan_priority] == 1 } {
								ixNet setA $filed -startValue $vlan_priority
								ixNet setA $filed -singleValue $vlan_priority
								ixNet commit
							} else {
								ixNet setA $filed -valueList $vlan_priority
							}
						}
						if { [ info exists vlan_priority_fullmesh ] } {
							ixNet setA $filed -fullMesh $vlan_priority_fullmesh
							ixNet commit
						}
						if { [ info exists vlan_priority_mode ] } {
							ixNet setA $filed -valueType $vlan_priority_mode
							ixNet commit
						}
						if { [ info exists vlan_priority_num ] } {
							ixNet setA $filed -countValue $vlan_priority_num
							ixNet commit
						}
						if { [ info exists vlan_priority_step ] } {
							ixNet setA $filed -stepValue $vlan_priority_step
							ixNet commit
						}
					}
				}					
			} elseif { [ regexp mpls $stack ] } {
				Deputs "Waseem etst8"
				foreach filed [ ixNet getList $stack field ] {
					 if { [ regexp -nocase "mpls.label.experimental" $filed ] } {
						if { [ info exists mpls_exp ] } {
							if { [llength $mpls_exp] == 1 } {
								ixNet setA $filed -startValue $mpls_exp
								ixNet setA $filed -singleValue $mpls_exp
								ixNet commit
							} else {
								ixNet setA $filed -valueList $mpls_exp
								ixNet commit
							}
						}
						if { [ info exists mpls_exp_fullmesh ] } {
							ixNet setA $filed -fullMesh $mpls_exp_fullmesh
							ixNet commit
						}
						if { [ info exists mpls_exp_mode ] } {
							ixNet setA $filed -valueType $mpls_exp_mode
							ixNet commit
						}
						if { [ info exists mpls_exp_num ] } {
							ixNet setA $filed -countValue $mpls_exp_num
							ixNet commit
						}
						if { [ info exists mpls_exp_step ] } {
							ixNet setA $filed -stepValue $mpls_exp_step
							ixNet commit
						}
					}
				}					
...
#Deputs "protocolTemplateList in GetProtocolTemp:$protocolTemplateList"
    set index 0
    foreach protocol $protocolTemplateList {
	   if { [ regexp -nocase $pro $protocol ] } {
#Deputs "protocol in GetProtocolTemp:$protocol， pro: $pro"
...
body Traffic::get_stats { args } {
	...
	if {[info exists drill_down_type] } {
		set targetRowFilter {::ixNet::OBJ-/statistics/view:"Traffic Item Statistics"/drillDown/availableTargetRowFilters:}
		append targetRowFilter \"Traffic\ Item=$traffic_name\"
		if {[regexp -nocase "precedence" $drill_down_type]} {
			set view [CreatePerDrillDownView precedence $targetRowFilter]
		} elseif {[regexp -nocase "vlanPriority" $drill_down_type]}  {
    ...
    set lossPercentIndex        [ lsearch -exact $captionList {Loss %} ]
...
Deputs "aveJitterIndex:$aveJitterIndex"    
	...
	} elseif { [regexp -nocase "vlanpriority" $tracking] } {
		set vlanPriorityIndex [ lsearch -exact $captionList {VLAN:VLAN Priority} ]
	} elseif { [regexp -nocase "mplsexp" $tracking] } {
		set mplsExpIndex [ lsearch -exact $captionList {MPLS:MPLS Exp} ]
	}
	...
	for {set i 1} {$i <= [ixNet getA $view/page -totalPages]} {incr i} {
        ixNet setA $view/page -currentPage $i
        ixNet commit
        ...
        foreach row $stats {
            ...
            if { [ lindex $row $traNameIndex ] != $traffic_name && ![info exists drill_down_type]} {
                continue
            }
            ...
            if { $tracking == "precedence" } {
                ...
                set statsItem   "vlanpriority"
                set statsVal    [ lindex $row $vlanPriorityIndex ]
                if { [info exists filter_value] && [lsearch $filter_value $statsVal] == -1 } {
                    continue
                }
                Deputs "stats val:$statsVal"
                ...
                set statsItem   "mplsexp"
                set statsVal    [ lindex $row $mplsExpIndex ]
                if { [info exists filter_value] && [lsearch $filter_value $statsVal] == -1 } {
                    continue
                }
                Deputs "stats val:$statsVal"
           ...
           set ret $ret[ GetStandardReturnBody $statsItem $statsVal ]
			...
			set statsItem   "loss_percent"
            set statsVal    [ lindex $row $lossPercentIndex ]
            Deputs "stats val:$statsVal"
           set ret $ret[ GetStandardReturnBody $statsItem $statsVal ]
...
body Traffic::CreatePerPortView { rxPort } {
    ...
    set tfilterHandle [ixNet getL $customView layer23TrafficItemFilter]
    if {$tfilterHandle == "" } {
        set tfilterHandle [ixNet add $customView layer23TrafficItemFilter]
    }
    #ixNet setA $customView/layer23TrafficItemFilter -trafficItemFilterIds $itemFId
    ixNet setA $tfilterHandle -trafficItemFilterIds $itemFId
    ixNet commit
    ...
    set pfilterHandle [ixNet getL $customView layer23TrafficPortFilter]
    if {$pfilterHandle == "" } {
        set pfilterHandle [ixNet add $customView layer23TrafficPortFilter]
    }
    #ixNet setA $customView/layer23TrafficPortFilter -portFilterIds [ list $filteredId ]
    ixNet setA $pfilterHandle -portFilterIds [ list $filteredId ]
...
body Traffic::CreatePerDrillDownView { drillDownType targetRow } {
	set view {::ixNet::OBJ-/statistics/view:"User Defined Statistics"}
	
	ixNet setA {::ixNet::OBJ-/statistics/view:"Traffic Item Statistics"/drillDown} -targetRowFilter $targetRow
	
	if {[regexp -nocase "precedence" $drillDownType]} {
		set targetDrillDownOption {Drill down per IPv4 :Precedence}
	} elseif {[regexp -nocase "vlanPriority" $drillDownType]} {
	...
	ixNet exec doDrillDown {::ixNet::OBJ-/statistics/view:"Traffic Item Statistics"/drillDown}
    return $view
}
...
# -- Header implmentation
body EtherNoFcsHdr::config { args } {
...
body Ipv6SegRoutingHdr::config { args } {
    global errorInfo
    global errNumber

    set tag "body Ipv6SegRoutingHdr::config [info script]"
Deputs "----- TAG: $tag -----"
...
Deputs "Pro: $pro"
    if { $pro != "ipv6routingtype4" } {
	   error "$errNumber(3) key:protocol value:$pro"
    }
    SetProtocol IPv6RoutingType4
	
    if { [ info exists seg_left ] } {
	   AddField segmentRoutingHeader.segmentsLeft
	   AddFieldMode Fixed
	   AddFieldConfig $seg_left
    }
	
	if { [ info exists first_seg ] } {
	   AddField segmentRoutingHeader.firstSegment
	   AddFieldMode Fixed
	   AddFieldConfig $first_seg
    }
	
	if { [ info exists seg_list ] } {
	   set index 13
	   foreach seg $seg_list {
		   AddField segmentRoutingHeader.segmentList.ipv6SID-$index 0 0 0 1
		   AddFieldMode Fixed
		   AddFieldConfig $seg
		   incr index
	   }
